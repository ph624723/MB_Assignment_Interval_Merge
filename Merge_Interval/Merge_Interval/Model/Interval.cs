...
namespace Merge_Interval.Model.Interval
{
    ...
    /// </summary>
    public class Interval<T> : IntervalBase<Interval<T>, T> where T : IComparable<T>
    {
        private T _start;
        private T _end;
        ...
        public Interval([NotNull] T start, [NotNull] T end)
        {
            // Check that internal fields will always be non-null and in correct order
        ...
        public override bool Intersects([NotNull] Interval<T> compareInterval)
        ...
        /// <param name="compareInterval">SafeInterval to compare to. Cannot be null or a NoNullAllowedException will be thorwn.</param>
        public override IEnumerable<Interval<T>> Union([NotNull] Interval<T> compareInterval)
        {
            // Check input is non-null
            ...
            return Intersects(compareInterval) ?
                ...
                new List<Interval<T>>() { new(ComparableMathExtensions.Min(Start, compareInterval.Start), ComparableMathExtensions.Max(End, compareInterval.End)) } :
                // No intersection -> return both intervals
                new List<Interval<T>>() { this, compareInterval };
        }
    }
}
